...
| `src/content/` | **the content source**: one YAML file per entry under `projects/`, `experience/`, `education/`, `certificates/`, `skills/`, `languages/`, and `profile.yaml`, with the certificate documents and their previews under `certificates/files/`. Every fact the site or the CV shows lives here and nowhere else; `src/content/README.md` documents the format |
