...
## Acceptance Criteria
- [ ] Each document page's actions row contains exactly one element; it is a link whose visible content is an icon, whose accessible name names the document it downloads, and whose `href` resolves to that page's own PDF (criterion 1).
- [ ] Neither document page contains a link to `resume.json` or to the other document's page, and the header navigation on both still links to the CV and to the resume (criterion 1).
- [ ] The control is reachable and activatable by keyboard alone, has a visible focus ring in both themes, and its tooltip and accessible name agree (criterion 8).
- [ ] Every string the control uses exists in English and Arabic, the strings the removed links used are gone from both, `pnpm check` reports no error, and the gap report does not grow (criterion 8).
- [ ] The row is still hidden in print, so neither the published PDF nor a generated one shows it (criterion 7).
- [ ] `pnpm test` passes with `tests/resume.spec.ts` updated: the actions-row test asserts one control per page rather than the old link set (criterion 1).
