...
- [x] `README.md` opens with a greeting naming Saud and carries the summary, the portfolio link in both languages, the CV link with its downloads, the email, and the skill groups, with emoji as profile READMEs use, and carries no build, deployment, or content-format documentation and no description of the repository (criterion 15, requirement 15).
  Verified 2026-09-09 by reading `README.md`: `# Hi, I'm Saud 👋`, then the generated block with the summary paragraph, `🌐 Portfolio` linking `/en/` and, as `بالعربية`, `/ar/`, `📄 CV` linking the page, `cv.en.pdf`, and `en/resume.json`, `✉️ Email`, and `## 🧰 What I work with` with the six skill groups; and nothing else. A `This repository` paragraph pointing at the documentation was there when the ticket first resolved; Saud asked on 2026-09-09 that the page be the facing text alone, and it went, with the spec's criterion 15 saying so.
