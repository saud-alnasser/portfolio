...
Drafted 2026-09-09 by a dispatched implementer and integrated as a draft: 115 `ar` lines added across 56 content files with no fact or English text changed, and six UI strings in `src/lib/i18n.ts` corrected (`period.present`, `cv.description`, the `certificate-pending` wording, `home.location`, `experience.highlights`, `project.live`). Nothing is published by this: the site deploys only when the effort merges, and the effort does not close while this ticket is open.

**Waiting on Saud.** The third criterion is his reading of every Arabic string. His reading copy is the review sheet the child wrote (one table per file, English beside Arabic, 153 rows), and the same text is on the effort branch's `/ar/` pages and in the files. When he has read it, record here the date and the commit he read, correct whatever he changes, and resolve the ticket.
