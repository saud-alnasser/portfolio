...
- **The resume is full.** Corrected on 2026-09-10, after a review tested the earlier wording that marking a certificate later would be one field on its entry: it is not. Appending the marker to one dateless course entry and rebuilding fails with `render-pdf: resume-too-long: en at A4 runs to 2 pages`. Marking a certificate, or adding a project to the resume, means shortening something else in the same change, and the render step refuses the build until it fits. Saud chose this over trimming further now to buy room that might never be used.
