...
| `src/components/CvDocument.astro` | the whole document for either variant, in the document order the extraction check reads; the `cv-compact` class on the resume variant |
...
| `src/styles/global.css` | the `cv-compact` print rules |
...
- **The resume does not fit one page.** The estimate at 10pt with the compact rules is about 45 of 57 available lines in English; Arabic runs longer and the bundled Naskh face sits taller. The render step fails naming the locale and the paper, and the levers are content, in the order the architecture section gives. If the Arabic resume cannot fit at 10pt with the content trimmed, that is a return-to-plan finding: the spec's one-page rule would need a per-language reading, which is Saud's call.
